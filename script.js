...
const catalog = [
  ...
  ['classic-burger','Classic S&S Burger','Burgers','₹549','House patty, cheddar, lettuce, secret sauce.','Non-veg','https://images.unsplash.com/photo-1568901346375-23c9450c58cd?auto=format&fit=crop&w=700&q=85&v=4'],
...
const renderCatalog = () => {
  ...
  dishGrid.innerHTML = visible.map((item) => `<article class="dish-card reveal"><div class="dish-image"><img src="${item.image}" alt="${item.name}"><span class="image-fallback" aria-hidden="true">✦</span></div><div class="dish-body"><div class="dish-meta"><span>${item.category} · ${item.tag}</span><strong>${money(item.price)}</strong></div><h3>${item.name}</h3><p>${item.description}</p><button class="add-button" data-id="${item.id}" data-name="${item.name}" data-price="${item.price}" data-image="${item.image}">Add to cart <span>+</span></button></div></article>`).join('');
  $('.no-results').hidden = visible.length > 0;
  $$('.reveal', dishGrid).forEach((element) => observer.observe(element));
  $$('img', dishGrid).forEach((image) => image.addEventListener('error', () => { image.closest('.dish-image').classList.add('image-failed'); image.removeAttribute('src'); }));
